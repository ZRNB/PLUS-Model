...
def tf(x):
    if isinstance(x, list):
        x = np.array(x)  
    return torch.as_tensor(x, dtype=torch.float32, device=_dev[0])
...
class GridEnv:
    ...
    def s2r(self, state):
        return self.func(self.s2x(state))
...
class TreeNode:
    def __init__(self, state_tuple, is_terminal_node):
        self._parent = []  # {parent_node: prior_p}
    ...
    def is_fully_expanded(self, env):
        possible_actions = set(range(env.ndim)) | {env.ndim}
        return set(self._children.keys()) == possible_actions
    
    def is_terminal(self, env):
        state = np.array(self.state_tuple)
        return state.max() >= env.horizon - 1
    
    def select(self, env, c_puct, agent):
        if self.is_terminal(env) or self.is_terminal_node:
            return self, -2
        if not self.is_fully_expanded(env):
            return self, -1 

       
        action_puct = {}
        for action, _ in self._children.items():
            action_puct[action] = self.get_value(c_puct, action, agent)
        
        actions = list(action_puct.keys())
        puct_values = np.array(list(action_puct.values()))
...
def create_node(parent, state_tuple, is_terminal_node):
    node = TreeNode(state_tuple, is_terminal_node)
...
class FlowNetAgent:
    ...
    def playout(self, env, func, root_node, path):
        state_tuple = tuple(env.get_state())
        node = state_node_map[state_tuple]
        env_copy = copy.deepcopy(env)
        count = 0
        while True:
            best_node, best_action = node.select(env_copy, c_puct, self)
            if best_action == -2: 
                reward = env_copy.s2r(np.array(node.state_tuple))
            ...
            path.append(best_node)
            count += 1
            observation, reward, done, _ = env_copy.step(best_action)
            node = best_node

        action_probs = []
        with torch.no_grad():
            logits = self.model(tf(env_copy.obs(env_copy.get_state())))
        ...
        # print(unexplored)
        if unexplored:
            for action, prob in unexplored:
                new_state = list(node.state_tuple)
                if action < env.ndim:
                    new_state[action] += 1 
                    new_state = tuple(new_state)
                    if new_state in state_node_map:
                        child2 = state_node_map[new_state]
                    else:
                        child2 = create_node(node, new_state, False)
                        state_node_map[new_state] = child2
                    node._children[action] = child2
                else: 
                    _, leaf_value, _, _ = env_copy.step(action)
                    new_state = tuple(new_state)
                    child = create_node(node, new_state, True)
                    node._children[action] = child
            
            actions, probs = zip(*unexplored)
            probs = np.array(probs)
            probs = probs / probs.sum()
            selected_idx = np.random.choice(len(actions), p=probs)
            best_action, best_prob = actions[selected_idx], probs[selected_idx]
            env_copy.step(best_action)
            path.append(node._children[best_action])
            leaf_value = self.evaluate_rollout(env_copy)
            
            (node._children[best_action]).update_recursive(leaf_value, path)
            for _ in range(count + 1):
                if path:
                    path.pop()
          

    def evaluate_rollout(self, env):
        cumulative_reward = 0
        state = env.get_state()
        if(state.max() >= env.horizon - 1):
            return env.s2r(np.array(env.get_state()))
        for i in range(max_deepth): 
            s = env.obs(state)
            with torch.no_grad():
                action = Categorical(logits=self.model(tf(s))).sample()
    ...
    def get_move(self, env, root_node, path, current_step):
        
        state_tuple = tuple(env.get_state())
        current_node = state_node_map[state_tuple]
        for _ in range(n_playout):
            self.playout(env, func_corners, root_node, path)
        visit_counts = [(child._n_visits**5) for action, child in current_node._children.items()]
        total_visits = sum(visit_counts)
        probs = [count/total_visits for count in visit_counts]

        with torch.no_grad():
            logits = self.model(tf(env.obs(env.get_state())))
            pf = torch.softmax(logits, dim=-1).cpu().numpy() 

        actions = list(current_node._children.keys())
        # epsilon = self.get_mcts_weight(current_step)
        u = [(1-epsilon) * pf[i] + epsilon * probs[i] for i in range(len(probs))]
        u_sum = sum(u)
        u = [x / u_sum for x in u]

        actions = list(current_node._children.keys())
        selected_action = np.random.choice(actions, p=u)

        env_copy = copy.deepcopy(env)
        sp,r,done,s=env_copy.step(selected_action)
        path.append(state_node_map[tuple(s)])
        return selected_action

    def sample_many(self, mbsize, all_visited, root_node, current_step):
        batch = []
        batch += self.replay.sample()
        s = tf([i.reset()[0] for i in self.envs])
        paths= [[] for _ in range (mbsize)]
        for i in range(mbsize):
            paths[i].append(state_node_map[tuple(self.envs[i].get_state())])
        done = [False] * mbsize
        while not all(done):
            # Note to self: this is ugly, ugly code
            active_envs = [e for d, e in zip(done, self.envs) if not d]
            active_indices = [idx for idx, d in enumerate(done) if not d]
            acts = []
            for i, e in enumerate(active_envs):
                acts.append(self.get_move(e, root_node, paths[active_indices[i]], current_step))
...
def compute_empirical_distribution_error(env, visited):
    ...
    # KL divergence
    # print(env.get_state());assert 0
    density_list = true_density.tolist()
    estimated_list = estimated_density.tolist()
    # for i in range(0, len(density_list), 8):
    #     row = density_list[i:i+8] 
    #     print(" ".join([f"{x:.3f}" for x in row])) 
    # for i in range(0, len(estimated_list), 8):
    #     row = estimated_list[i:i+8] 
    #     print(" ".join([f"{x:.3f}" for x in row])) 
...
def main(args):
    ...
    envs[0].reset()
    node_state = tuple(envs[0].get_state())
    root_node = create_node(None, node_state, False)
    state_node_map[node_state] = root_node

    for i in tqdm(range(args.n_train_steps+1), disable=not args.progress):
        data = []
        for j in range(sttr):
            data += agent.sample_many(args.mbsize, all_visited, root_node, i)
        for j in range(ttsr):
        ...
        if i%100 == 0:
            ...
            print(f"[Resource] Step {i}: CPU {cpu_usage:.1f}% | RAM {mem:.2f} GB | GPU {gpu_mem} MB", flush=True)
            ...
            print('epoch ',i,' loss', np.mean(losses[0].item()), 'nums of mode found is ',num_modes, 'len of visited is ',len(all_visited))
